...
//2.    iterate through the list and look for the requried id  26335A

//        declare a boolean that will be a flag to control
...
//                  we found the desired row, so we need to end the while loop
                    notFound=false;
//                 we found the desired row, so we do not want to iterate the rest
//                 of the list of webElements and waste our time
                    break;
                }
            }
//    click on next btn once the loop has traversed
//    the whole list (which contains all the entries of the current page)
//      but only click on next if you still haven't found the required id
            if(notFound) {
                WebElement nxtBtn = driver.findElement(By.xpath("(//a[text()='Next'])[1]"));
                nxtBtn.click();
            }
